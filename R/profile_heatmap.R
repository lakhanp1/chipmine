...
#' argument geneGroups
#'
#' @param profileMat profile matrix of class normalizedMatrix
#' @param signalName name of the signal track
#' @param columnTitle title to be placed on the column. Default: signalName
#' @param createClusters Binary: whether or not to do kmeans clustering. If FALSE,
...
#' Two columns must be present in this dataframe: 'gene', 'cluster'. In case it is NULL, heatmap
#' rows are not split into different groups.
#' @param profileColor Color function for profile heatmap generated by colorRamp2
...
#' @param ylimFraction A numeric vector with one or two elements for deciding the ylimit of top
#' profile heatmap annotation. If the value is single number, it has to be floating point number
#' to extract the quantile and use limit [0, quantile(x)]. If the value is numeric vector of
...
#' \item rowAnno: row annotation object
