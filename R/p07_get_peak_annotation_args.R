...
get_peak_annotation_args = function(anName){
  ## peak types
  peakAnnotationCol = c("include_tx" = "#e41a1c", "include_CDS" = "#e41a1c",
  ...
  peakAnnotationLgd = Legend(title = "\nPeak type",
                             at = names(peakAnnotationCol),
                             type = "points",
                             pch = 15,
                             size = unit(5, "mm"),
                             grid_height = unit(5, "mm"),
                             grid_width = unit(5, "mm"),
                             legend_gp = gpar(col = peakAnnotationCol),
  ...
  # grid.rect()
  # draw(peakAnnotationLgd, x = unit(0.5, "npc"), y = unit(0.5, "npc"))
  # popViewport()

  return(list(
    col = list(peakAnnotationCol) %>% purrr::set_names(nm = anName),
    lgd = peakAnnotationLgd
  ))
