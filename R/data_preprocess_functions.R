...
gene_level_peak_annotation <- function(
  sampleId, peakAnnotation, cdsFile, peakFile, bwFile,
  preference = c("nearStart", "peakInFeature", "featureInPeak", "nearEnd", "upstreamTss"),
  outFile = NULL,
  bindingInGene = FALSE, removePseudo = TRUE,
  ...
  # allPeakCategories <- c("nearStart", "peakInFeature", "featureInPeak", "nearEnd", "upstreamTss")
  ...
  peakAnnDf <- dplyr::left_join(x = peakAnnDf, y = peakCovDf, by = structure("name", names = tfCols$peakId[1])) %>%
  ...
  finalDf <- dplyr::left_join(x = geneSet, y = filteredPeaks, by = c("gene" = "geneId"))  %>%
    ...
    dplyr::select(1:5, !! tfCols$hasPeak, !! tfCols$peakPosition, tfCols$peakType, tfCols$peakId,
                  tfCols$peakCategory, tfCols$preference, everything(), -promoterCov) %>%
  ...
  finalDf <- dplyr::group_by(finalDf, gene, !! as.name(tfCols$peakPosition)) %>%
    dplyr::arrange(!!as.name(tfCols$preference), desc(!!as.name(tfCols$peakDist)), .by_group = TRUE) %>%
    dplyr::slice(1L) %>%
    dplyr::ungroup() %>%
...
#' "peakPosition", "peakType", "peakId", "peakEnrichment", "peakPval", "peakQval",
