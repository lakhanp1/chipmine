...
gene_level_peak_annotation <- function(
  ...
  outFile = NULL, removePseudo = TRUE,
  ...
  peakAnnDf <- dplyr::mutate(peakAnnDf, !! tfCols$hasPeak := TRUE)

  finalDf <- dplyr::left_join(x = genesDf, y = peakAnnDf, by ="geneId")  %>%
    dplyr::mutate(
  ...
  finalDf <- dplyr::group_by(finalDf, geneId) %>%
    dplyr::arrange(!!sym(tfCols$preference),
                   # abs(!!sym(tfCols$peakDist)),
                   desc(!!sym(tfCols$peakEnrichment)),
                   .by_group = TRUE) %>%
    dplyr::slice(1L) %>%
    dplyr::ungroup()
...
#'
#' @examples NA
import_peak_annotation <- function(sampleId, peakAnnoFile, removePseudo = TRUE,
                                   fcCutoff = 1, pvalCutoff = 1,
