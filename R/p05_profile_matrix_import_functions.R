...
#' from the profile matrix for plotting.Default: NULL
...
#'
#' @examples NA
import_profile_from_file = function(file, source = "normalizedmatrix", signalName,
                                    selectGenes = NULL,
  ...
  getSubProfile <- FALSE
  if (is.character(selectGenes)) {
    getSubProfile <- TRUE
    geneDf <- tibble::tibble(geneId = selectGenes)
  } else if(!is.null(selectGenes)){
    stop("selectGenes should be a character vector of gene IDs")
  }
  ...
  if(getSubProfile){
    profileDf <- dplyr::left_join(x = geneDf, y = profileDf, by = c("geneId" = "geneId"))
  }
  
  profileDf <- tibble::column_to_rownames(.data = profileDf, var = "geneId")
...
#'
#' @examples NA
import_profiles <- function(exptInfo, ...){
